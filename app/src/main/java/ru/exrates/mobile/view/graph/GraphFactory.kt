...
class GraphFactory(private val anyChartView: LineChartView, val currentInterval: String) {
    ...
    private fun createChartValueDataList(priceHistory: List<Double>): ValueDataList {
        ...
        if (!autoGeneratedY){
            val decimalPattern = when{
                priceHistory[0] < 10 -> "#.########"
                priceHistory[0] >= 10 && priceHistory[0] < 100 -> "##.#######"
                priceHistory[0] >= 100 && priceHistory[0] < 1000 -> "###.######"
                priceHistory[0] >= 1000 && priceHistory[0] < 10000 -> "####.#####"
                priceHistory[0] >= 10000 && priceHistory[0] < 100000 -> "#####.####"
                priceHistory[0] >= 100000 && priceHistory[0] < 1000000 -> "######.###"
                else -> "#######.##"
            }
            val f = DecimalFormat(decimalPattern)
            f.maximumFractionDigits = decimalPattern.split(".")[1].length
